...
export default {
	...
	async scheduled(_event, env, _ctx): Promise<void> {
		const playingStatusRes = await fetch(`https://api.listenbrainz.org/1/user/${env.LISTEN_BRAINZ_USER_NAME}/playing-now`)
		...
		const playingStatus = await playingStatusRes.json() as ListenBrainsResponse
		console.log(JSON.stringify(playingStatus))

		const metadata = playingStatus.payload.listens.length > 0 ? playingStatus.payload.listens[0].track_metadata : null
		let postText = metadata
			? `${metadata.artist_name} - ${metadata.track_name} ${ metadata.release_name
				? `(from: ${metadata.release_name})`
				: '' }`
			: 'nothing playing!'
		...
		if (!pageRes.ok) {
			console.error(`cannot fetch page!: ${JSON.stringify(await pageRes.json())}`)
			return
		}
		...
		try {
			const postRes = await fetch(`https://${env.MISSKEY_HOST}/api/pages/update`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
